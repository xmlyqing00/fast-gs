...
def sample_data(in_dir: Path, out_dir: Path, sample: int):

    if not out_dir.exists():
        out_dir.mkdir(parents=True)

    img_dir = in_dir / 'rgb'
    depth_dir = in_dir / 'depth'

    print(img_dir,' ')
    print(depth_dir,' ')

    img_list = sorted(img_dir.glob('*.tiff'))
    # depth_list = sorted(depth_dir.glob('*.tiff'))

    img_out_dir = out_dir / 'image'
    img_out_dir.mkdir(parents=True, exist_ok=True)

    for i in trange(0, len(img_list), sample):
        img = cv2.imread(str(img_list[i]), cv2.IMREAD_UNCHANGED)
        cv2.imwrite(str(img_out_dir / f'{img_list[i].stem}.png'), img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default='//IVC-LAB-NAS/home/lyq/robot-3d/Spot-RGBD-sample/2024-08-15-15-22-43')
    parser.add_argument('--output', type=str, default='G:/Dataset/robot-3d')
    ...
    # sample_data(in_dir, out_dir, args.sample)
    dtu_to_json(out_dir)
